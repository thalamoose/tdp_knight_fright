#CSpect v3.0.15.2 ZXSpectrum emulator by Mike Dailly
(c)Copyright 1998-2025 All rights reserved

Be aware...this emulator is far from well tested, and might crash for any reason - sometimes just out of pure spite!
...
v3.0.15.1
---------
Fixed a typo in DeZog plugin causing a hang
 ...
 * void PokePhysicalULA(int _address, byte _value);
 Updated DeZog plugin to use new ULA peek/poke so loading works 
 Fixed tilemap setting when address is $0000-$1FFF and bank is 10,11 or 14
...
Added "-dscale" command line option, to double the size of the debugger text (incase you find bits hard to read, also remember "-thin")
...
Updated mainloop mid line audio "tick" to help stop mistaken extra calls
...
CTRL+U now toggels showing UNDER the ULA memory in "physical" memory dump window (when using m bank:offset mode)
