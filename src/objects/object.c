...
{
	u8 animIndex = pObject->anim.frameIndex+pObject->anim.baseIndex;
    if (animIndex != pObject->anim.lastIndex)
    {
        pObject->anim.lastIndex = animIndex;
        u8 page = (animIndex>>3)+pObject->anim.sprite.page;
        nextreg(MMU_SLOT_6, page);
        u8 *pPattern = (u8 *)SWAP_BANK_0+((animIndex&7)<<10);
		//x_printf("%x: src:%x,idx:%d,cnt:%d\n", pObject, pPattern, (u16)pObject->anim.sprite.pattern, (u16)pObject->anim.sprite.patternCount);
        CopySprite(pPattern, pObject->anim.sprite.pattern, pObject->anim.sprite.patternCount);
    }
