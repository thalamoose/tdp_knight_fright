...
void HandlePickup(game_object* pObject)
{
    ...
    config.baseIndex = PLAYERSPR_IDLE_ANIM;
...
{
    {PLAYERSPR_L+PLAYERSPR_RUN_ANIM, PLAYERDIR_BL,-FIXED_POINT_ONE, -FIXED_POINT_ONE*2},
    ...
    {PLAYERSPR_D+PLAYERSPR_RUN_ANIM, PLAYERDIR_TL, FIXED_POINT_ONE, -FIXED_POINT_ONE*5}
};
...
void CollidePlayer(game_object* pPlayer, const game_object* pCollider)
{
    ...
    play_cell* pCell = GetPlayAreaCell(pCollider->playGrid.x, pCollider->playGrid.y);
    switch (pCell->type)
    {
        case CELL_HOLE:
        {
