...
{
    pAnim->sprite.page = animPage;
    pAnim->sprite.patternCount = spriteCount;
    pAnim->sprite.centerOffset = *centerOffset;

	for (u8 i=0; i<pAnim->sprite.patternCount; i++)
	{
		sprite_config spriteConfig = configs[i];
		pAnim->sprite.slot[i] = AllocSpriteSlot();
		pAnim->sprite.pattern[i] = AllocSpritePattern();
		spriteConfig.pattern = pAnim->sprite.pattern[i];
		spriteConfig.attr2 |= pAnim->sprite.palette<<4;
        SetupSprite(pAnim->sprite.slot[i],  &spriteConfig);
	}
}
